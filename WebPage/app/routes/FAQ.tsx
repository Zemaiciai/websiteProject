...
const FAQ = () => {
  ...
  const [searchNotFound, setSearchNotFound] = useState(false); // State to track if search is not found

  const handleTabClick = (tab: string) => {
    setActiveTab(tab);
  };

  // Filter FAQ messages based on search query
  useEffect(() => {
    if (!searchQuery) {
      setFilteredFAQMessages(faqMessages);
      setSearchNotFound(false); // Reset search not found when search query is empty
    } else {
      ...
      setSearchNotFound(filtered.length === 0); // Set search not found if filtered array is empty
    }
